...
- Pan/tilt head servos on GP20 (pan) / GP21 (tilt) — stable with a 2° target deadband (`SERVO_DEADBAND` in `app_control.py`) and 1°/5ms-tick interpolation (~200°/s). Dashboard arrow keys send 2° steps every 50 ms (~40°/s) so the Pico stays caught up between sends and motion stops within ~1° of key release. `set_angle` is only re-issued when the interpolated angle actually changes — writing the same PWM 200×/s caused observable micro-twitch.
