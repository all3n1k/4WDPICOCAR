...
- **MPU6050 IMU**: [OPERATIONAL — properly calibrated as of 2026-04-30] — wired to I²C0 (GP0/GP1) at 0x68. Driven via SoftI²C (HW I²C peripheral on MicroPython 1.28.0 EIOs on clock-stretched reads; SoftI²C handles stretching cleanly, costs ~1ms/tick). Full-stack tuning landed: (1) Gyro full-scale range = ±1000°/s (FS_SEL=2, scale=32.8 LSB/°/s). Earlier ±250 and ±500 SATURATED during powered turns on this 4WD chassis (counter-rotating wheels hit 800-1200°/s peaks); the gyro silently clipped and the integrated heading captured ~55-71% of real rotation, accumulating 180° of phantom drift after ~6-7 turns. ±1000°/s eliminates saturation. (2) Boot calibration is 500 samples × 5ms = 2.5s of stillness; bot must NOT move during this window. (3) Online auto-bias correction: when both `gyro_z < 0.3°/s` AND accelerometer magnitude is within 5% of 1g for 50 consecutive ticks (~250ms), bias is nudged toward the current raw value (EMA α=0.005). The accel check is critical — earlier versions with gyro-magnitude-only stillness mis-classified post-turn coast-down as "still" and absorbed real motion into bias, causing the IMU's reference frame to drift 180° after several powered turns. (4) Heading deadzone = 0.75°/s — empirically tuned: at 1.0 the integration lost real slow-ramp motion (~9° per 360°); at 0.5 residual bias leaked through (~10° per 360°). (5) Sign convention: `gyro_z` is negated at source so heading is **compass-CW-positive** (E=90, S=180, W=270) — without that flip a hand-rotated 360° loop counted backwards. Hand-rotation loop closes within 3-9° per 360°; powered turns within 5-15° (noise floor for consumer MPU6050 without magnetometer). For tighter, add HMC5883L on same I²C0 bus for absolute compass anchor.
...
- **Shift = boost** (dashboard manual control). Holding Shift while pressing W/A/S/D suffixes the command with `_boost`; the agent strips the suffix and dispatches at `BOOST_SPEED=100` (motor 100% duty) instead of `MANUAL_SPEED=75` (motor 80% duty). The Pico's motor mapping is `duty% = 20 + 0.8 × input` (`libs/pico_rdp.py:Motor.power.setter`) — the 20% floor overcomes static friction. `TURN_SPEED=70` (motor 76%). Shift state is tracked separately and can be toggled mid-press: pressing W → adding Shift → releasing Shift seamlessly transitions because `startManual` re-sends the active direction every 80 ms. Bottom LED tint switches to magenta when boosting (cyan = normal manual). Browser-tab-blur handler clears all held keys + sends `stop` so the robot doesn't run away if focus is lost mid-press. **Critical regression note:** `BOOST_SPEED` and the `_boost` suffix handler in `agent.py:_control_loop` have been reverted by a linter/auto-formatter pass at least twice. If Shift+W silently does nothing, check that line `BOOST_SPEED = 100` is still in the constants and the `elif manual.endswith("_boost"):` branch is in the manual-cmd drain.
...
- Dashboard: live video (`object-fit:cover` to kill black bars on mismatched stream resolutions) with YOLO + ArUco HUD, radar polar plot, 700×700 floor plan with robot pose / trail / discovered markers / obstacle cloud (heading arrow rendered with `APARTMENT_FRAME_OFFSET_DEG=205` so it matches real-world direction — the floorplan PNG was captured rotated relative to compass-true), action log, WASD + arrow-key pan/tilt, click-to-place, shift-click to pin a marker. Brain panel scrolls correctly on short windows. Laser tile shows `OOR (>2m / no return)` instead of `---` when the VL53L0X has no reflection — clearer that the path is open vs the sensor being broken.
- **`--auto` flag (opt-in autonomy)**: by default, `python3 main_os.py` runs the dashboard, manual control, sensor streams, and reflex safety thread — but the **LLM brain (think loop) does not start**. This makes the robot safe for testing and calibration without the model immediately trying to drive. Pass `--auto` to activate the autonomous LLM. Wired in `main_os.py` (`AUTO_MODE`) and `agent.py:AgentCoreModule.start` (the `_think_loop` Thread is gated on `self._auto`).
- **Boot resets sensor state by default**: every `main_os.py` boot wipes `semantic_map.json` (obstacle cloud) and `constellation_map.json` (auto-discovered marker positions). The robot may have been physically picked up and moved while powered down, so prior runtime sensor data is presumed stale. `room_markers.json` (manual wall-marker survey) and `apartment_map.py` (static apartment prior) are always preserved. Pass `--continue` to keep last session's runtime state.
- **Reflex thread (safety) with multi-tick confirm**: 25 Hz check; triggers stop only after **3 consecutive ticks** of close readings (`COLLISION_THRESHOLD=27` cm forward, `COLLISION_EMERGENCY=12` cm, `COLLISION_WIDE_DIST=10` cm). The 3-tick confirm is critical: laser/sonar false-positives on glazed-floor specular reflection give 1-2 isolated ghost-close readings (18-27 cm) that previously fired the reflex on every tick. Each false stop raced manual `forward` commands on the WS pipe, causing motor stutter and "controls feel terribly late + overshooting." After confirm: hold 200 ms minimum + clear 3 consecutive non-close ticks before re-enabling. Real obstacles still trigger reliably (they keep producing close readings); single-tick noise gets silently filtered.
- **Dashboard `_sync_dashboard` rate-limited to 10 Hz** (was firing every main-loop iteration ≈ 100 Hz). Each call iterates the 5041-cell semantic grid via `get_obstacles()` and writes through `state.lock`; running it 10× faster than the dashboard's 3 Hz JS poll was pure CPU waste and produced compounding input lag as a session continued. Implemented in `agent.py:AgentCoreModule._sync_dashboard` via `_last_sync_ts` + `_SYNC_MIN_INTERVAL` gate.
- **Manual command queue**: `state.manual_cmds` in `dashboard.py` is bounded to ≤8 entries; on overflow, drops oldest. `stop` is **kill-switch priority** — the moment a `stop` command arrives at `/api/control`, the queue is replaced with `['stop']` instead of FIFO-appended. Without this, releasing W produced `[forward, forward, ..., stop]` which drained in order and the robot ran forward for the full drain duration after key release.
- **TTS (macOS `say`) with drop-if-busy semantics**: `_speak_async` uses `Popen` and tracks `_speak_proc`. New utterances while one is still playing are **silently dropped** (not interrupted — macOS speech-synthesis daemon keeps queued audio playing past SIGTERM, causing voice overlap). Only one voice plays at a time. Auto-speak of inner thought and move/turn results was REMOVED — only explicit `speak` tool calls vocalize. Earlier auto-speak paths flooded `say` with overlapping requests.
- LLM brain: think thread (gated on `--auto`), reflex safety thread, control loop, tool-calling JSON (`move`/`turn`/`look`/`scan`/`speak`/`set_mood`/`tune_parameters`/`patch_self`/`reboot`). **Model**: `gemma4:26b` on `digitalstorm:11434` (was 31b — swapped down for ~2× faster ticks). Sampling per Gemma model card: temp=1.0, top_p=0.95, top_k=64.
...
# Normal mode — dashboard + manual + sensors + reflex. NO LLM brain by default.
# Wipes prior runtime state (semantic_map.json, constellation_map.json) on boot.
python3 main_os.py
open http://localhost:8080

# Add --auto to actually start the autonomous LLM brain (think loop):
python3 main_os.py --auto

# Add --continue to keep last session's runtime state (skip the boot wipe):
python3 main_os.py --continue
python3 main_os.py --auto --continue   # both, for resuming an autonomous run
...
_Last snapshot: 2026-04-30 (v5.8.1-baseline — IMU calibration finally landed: gyro range to ±1000°/s killing saturation on powered turns, accel-magnitude-gated stillness detection killing bias pollution from coast-down, sign flip for compass-CW convention, 0.75°/s deadzone empirically tuned, 2.5s boot calibration. Reflex thread got 3-tick consecutive confirm + 200ms hold (kills false-positive stop-spam from glazed-floor sensor ghosts that were racing manual forwards on the WS pipe and producing "controls feel terribly late + overshoot"). `_sync_dashboard` rate-limited to 10Hz (was running at 100Hz from the main control loop, eating CPU on per-tick `get_obstacles()` over the 5041-cell grid). Manual queue gets stop-priority kill-switch in `/api/control`. Boot wipes runtime sensor state by default (`--continue` to keep). New `--auto` flag — LLM brain is opt-in; default mode is dashboard + manual + reflex + sensors with no autonomous reasoning, safe for testing. Model swap to `gemma4:26b` with Gemma sampling defaults. TTS drop-if-busy + auto-speak removed. Dashboard: video `object-fit:cover` (no black bars), brain panel scrolls, laser tile shows `OOR` instead of `---`, floorplan arrow uses `APARTMENT_FRAME_OFFSET_DEG=205` so visual direction matches real-world. Mapping workstream (M1+) still pending; the localization layer is now the next target now that the IMU is honest._
