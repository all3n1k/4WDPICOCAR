...
class VL53L0X:
    ...
    def read_mm(self):
        """Canonical Pololu-port read — returns raw distance in mm, raises TimeoutError."""
    ...
    def read(self):
        """Firmware-facing read — returns distance in cm (float) or None on
        timeout/out-of-range. 8190 mm is the chip's "no return" sentinel."""
        try:
            mm = self.read_mm()
